...
async function apiCall<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      else if (response.status === 422) errorCode = 'VALIDATION_ERROR';
      else if (response.status >= 500) errorCode = 'SERVER_ERROR';
      
      throw new ApiClientError(errorCode, errorData.message || `HTTP ${response.status}: ${response.statusText}`);
    }
...
export async function submitFeedback(applicationId: string, rating: number, comment: string): Promise<import('../types').Feedback> {
